...
class sim():
    ...
    def fit(self, alpha_max, kappa, delta, eta0=1., norm= True, noise= 0., lazy= False, tol=1e-10, save_log= False, save_folder= 'results/d32/', save_key= ''):
        ...
        if self.lin_coeffW0:
            if lazy:
                folder_id = '%s_d%s_p%s_k%s_kappa%.5f_delta%.5f_eta0%.5f_noise%s_norm%s_orthWt%s_lincombW0%s_lazy%s' % (region, print_d, '{:03}'.format(self.p), '{:03}'.format(self.k), kappa, delta, eta0, '{:.0e}'.format(noise), str(norm), str(self.orthWt), str(self.lin_coeffW0), str(lazy))
            else:
                folder_id = '%s_d%s_p%s_k%s_kappa%.5f_delta%.5f_eta0%.5f_noise%s_norm%s_orthWt%s_lincombW0%s' % (region, print_d, '{:03}'.format(self.p), '{:03}'.format(self.k),  kappa, delta, eta0, '{:.0e}'.format(noise), str(norm), str(self.orthWt), str(self.lin_coeffW0))
        else:
            if lazy:
                folder_id = '%s_d%s_p%s_k%s_kappa%.5f_delta%.5f_eta0%.5f_noise%s_norm%s_orthWt%s_lazy%s' % (region, print_d, '{:03}'.format(self.p), '{:03}'.format(self.k),  kappa, delta, eta0, '{:.0e}'.format(noise), str(norm), str(self.orthWt), str(lazy))
            else:
                folder_id = '%s_d%s_p%s_k%s_kappa%.5f_delta%.5f_eta0%.5f_noise%s_norm%s_orthWt%s' % (region, print_d, '{:03}'.format(self.p), '{:03}'.format(self.k),  kappa, delta, eta0, '{:.0e}'.format(noise), str(norm), str(self.orthWt))
